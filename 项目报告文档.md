...
**类关系图**
┌─────────────────┐
│ MainWindow │ ← UI层（布局、事件响应、托盘、热键）
├─────────────────┤
│ Tracker │ ← 采集层（窗口信息获取、计时）
├─────────────────┤
│ Database │ ← 存储层（SQLite 读写）
├─────────────────┤
│HourlyChartWidget│ ← 图表层（自绘柱状图）
└─────────────────┘
...
1. `GetForegroundWindow()` 获得窗口句柄 `HWND`。  
2. `GetWindowThreadProcessId()` 获得进程 ID。  
3. `OpenProcess()` + `QueryFullProcessImageNameW()` 获得可执行文件完整路径。  
4. 从完整路径中提取文件名作为应用名（`currentAppName()`）。  
5. `GetWindowTextW()` 获得窗口标题（`currentWindowTitle()`）。
