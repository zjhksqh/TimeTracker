...
void MainWindow::setupUI()
{
    ...
    topBar->setStyleSheet(QStringLiteral(
        "background-color: rgb(28,28,30); border-bottom: 1px solid rgb(48,48,52);"));
    auto *topLayout = new QHBoxLayout(topBar);
    topLayout->setContentsMargins(12, 6, 12, 6);
    topLayout->setSpacing(8);

    // 每日按钮
    m_dailyButton = new QPushButton(QStringLiteral("每日"), topBar);
    m_dailyButton->setCheckable(true);
    m_dailyButton->setChecked(true);
    m_dailyButton->setCursor(Qt::PointingHandCursor);
    m_dailyButton->setStyleSheet(QStringLiteral(
        "QPushButton {"
    ...
    // 近7天按钮
    m_weeklyButton = new QPushButton(QStringLiteral("近7天"), topBar);
    m_weeklyButton->setCheckable(true);
    m_weeklyButton->setCursor(Qt::PointingHandCursor);
    m_weeklyButton->setStyleSheet(m_dailyButton->styleSheet());   // 复用样式
    ...
    topLayout->addWidget(m_dailyButton);
    topLayout->addWidget(m_weeklyButton);
    topLayout->addStretch();   // 弹簧，把设置按钮推到最右边
    ...
    connect(m_settingsButton, &QPushButton::toggled, this, [this](bool checked) {
        ...
        m_settingsButton->setText(checked ? QStringLiteral("返回") : QStringLiteral("设置"));
    });
...
QWidget *MainWindow::createSettingsPage()
{
    ...
    title->setStyleSheet(QStringLiteral("font-size: 18px; font-weight: 600; color: rgb(235,235,240);"));
...
void MainWindow::refreshUsageData()
{
    ...
    for (const UsageRecord &record : todayRecords) {
        ...
        if (!record.appPath.isEmpty()) m_appPathByName[record.appName] = record.appPath;
    ...
    for (int i = 0; i < 7; ++i) {
        ...
        if (topApps.size() > 3) topApps.resize(3);
        if (!topApps.isEmpty()) m_weeklyTopAppsByDay[i] = topApps;
    }
    ...
    for (int hour = 0; hour < dailySecondsByHour.size(); ++hour) {
        ...
        if (!list.isEmpty()) m_dailyTopAppsByHour[hour] = list;
    }
...
{
    if (!m_hourlyChartWidget || !m_primaryStatLabel) return;
    ...
    for (int i = 0; i < 24; ++i) hourLabels.append(QStringLiteral("%1时").arg(i));
...
void MainWindow::updateWeeklySummary()
{
    ...
    m_primaryStatLabel->setText(QStringLiteral("近7天总时长 %1        日均时长 %2")
    ...
    for (int i = 0; i < 7; ++i) dayLabels.append(start.addDays(i).toString("MM-dd"));

    int maxWeeklyMinutes = 30;
    for (int value : m_weeklyMinutesByDay) maxWeeklyMinutes = qMax(maxWeeklyMinutes, value);
    int dynamicMax = 120, tick = 30;
    if (maxWeeklyMinutes <= 120) {
        dynamicMax = ((maxWeeklyMinutes + 29) / 30) * 30; tick = 30;
    } else if (maxWeeklyMinutes <= 360) {
        dynamicMax = ((maxWeeklyMinutes + 59) / 60) * 60; tick = 60;
    } else if (maxWeeklyMinutes <= 720) {
        dynamicMax = ((maxWeeklyMinutes + 119) / 120) * 120; tick = 120;
    } else {
        dynamicMax = ((maxWeeklyMinutes + 179) / 180) * 180; tick = 180;
    }
...
QString MainWindow::formatDuration(int seconds) const
{
    ...
    if (hours > 0) return QStringLiteral("%1小时%2分钟").arg(hours).arg(minutes);
...
QIcon MainWindow::iconForApp(const QString &appName) const
{
    ...
    if (!directPath.isEmpty() && QFile::exists(directPath)) return provider.icon(QFileInfo(directPath));
    const QString executablePath = QStandardPaths::findExecutable(appName);
    if (!executablePath.isEmpty() && QFile::exists(executablePath)) return provider.icon(QFileInfo(executablePath));
...
void MainWindow::setupTrayIcon()
{
    ...
    QIcon icon(":/images/tubiao.png");           // 从 qrc 加载你的图标
...
{
    if (!m_usageSplitter || m_usageSplitter->count() < 2) return;
